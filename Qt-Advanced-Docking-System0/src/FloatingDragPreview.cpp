...
void FloatingDragPreviewPrivate::createFloatingWidget()
{
	...
	if (FloatingWidget)
	{
		...
		if (!CDockManager::testConfigFlag(CDockManager::DragPreviewHasWindowFrame))
		{
            ...
            if(DockWidget)
            {
                 rect = DockWidget->getPreRect();
            }
            else
            {
                rect = FloatingWidget->topLevelDockWidget()->getPreRect();
                qDebug()<<"rect height:"<<rect.height()<<"rect width:"<<rect.width();
            }
            if(rect.height()== 0 || rect.width() == 0)
            {
                ...
                FloatingWidget->setGeometry(FixedGeometry.left(),FixedGeometry.top(),rect.width(),rect.height()+2);
		}
	}
}
